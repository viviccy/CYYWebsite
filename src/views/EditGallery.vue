  ...
  methods: {
    ...
    async afterComplete(file) {
      ...
      try {
        ...
        //reference to main image location in firestore storage
        const imageRef = storageRef.child(
          `documents/GalleryPhotos/PrimaryPhoto/${newFileName}`
        )

        //create an empty canvas to put the image. This canvas will not be showing on page.
        var canvas = document.createElement("canvas")

        //Define the context. Required when creating canvas.
        let ctx = canvas.getContext("2d")

        //use 'FileReader' to convert the file to canvas blob
        var reader = new FileReader()

        /* File Reader 'onload' is executed through calling 'reader.readAsDataURL(file)' at #anchorReader */
        //using method 'readAsDataURL' will produce a an output of base64 URL when 'reader.result' or 'event.target.result' is used inside 'reader.onload'
        reader.onload = function(event) {
          ...
          /*
          Image() object 'onload' is executed through calling 'img.src = event.target.result' at #anchorImgSrc where an image is defined to load in.
          event.target.result = a base64 image URL generated by 'reader.onload' through 'event'.
           */
          img.onload = function() {
            /*  img.naturalWidth = the actual width of the image without any css (the actual width of the image in user computer) */
            if (img.naturalWidth > 1200) {
              //define the maximum width for the image to prevent uploading image with big file size
              canvas.width = 1200
            } else {
              canvas.width = img.naturalWidth
            }

            //calculate the canvas height based on the defined canvas width
            canvas.height = (img.height * canvas.width) / img.width

            //draw the image on canvas
            ctx.drawImage(img, 0, 0, canvas.width, canvas.height)

            //turn the canvas image into blob so that we can use the blob to upload to firebase storage
            canvas.toBlob(async function(blob) {
              //execute the task to put the cover image in firebase storarge
              await imageRef.put(blob).on(
                "state_changed",
                (snapshot) => {
                ...
                async () => {
                  ...
                  //once successfully uploaded to firebase, get the reference to the main image URL using firebase command '..getDownloadURL'. This will be use later at #galleryFirebaseDocument to store the image URL to firestore Gallery database.
                  imageURL = await imageRef.getDownloadURL()

                  //create a separate canvas for thumbnail
                  var canvasThumb = document.createElement("canvas")

                  //get the canvas context
                  let ctxThumb = canvasThumb.getContext("2d")

                  //define an new FileReader for the thumbnail.
                  var readerThumb = new FileReader()

                  /*This File Reader 'onload' is executed through calling 'readerThumb.readAsDataURL(file)' at #anchorReaderThumb */
                  readerThumb.onload = function(event) {
                    //create an empy image object for the thumbnail
                    let imgThumb = new Image()

                    //to put the base64 image to canvas
                    imgThumb.onload = function() {
                      canvasThumb.width = 400
                      canvasThumb.height =
                        (imgThumb.height * canvasThumb.width) / imgThumb.width
                      ctxThumb.drawImage(
                        imgThumb,
                        0,
                        0,
                      ...
                      //convert the thumbnail canvas to blob
                      canvasThumb.toBlob(async function(blobThumb) {
                        const storageRef = firebase.storage().ref()
                        ...
                        await docRefThumbnail.put(blobThumb).on(
                          "state_changed",
                          (snapshot) => {
                            console.log(snapshot)
                          },
                          ...
                          async () => {
                            console.log("load thumbnail image success")

                            /*  This will be use later at #galleryFirebaseDocument to store the thumbnail image URL to firestore Gallery database. */
